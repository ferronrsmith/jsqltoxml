...
public class ConfigurationTest {
    ...
    public void testCountRegexPasses() throws Exception {
        ...
        validateRegex(
                "count(//country[( name/text()='India' and food/text()='roti' )])", COUNT_REGEX);
    }
    ...
    public void testLoadConfigurationFile() throws Exception {
        assertEquals("^(count\\(//)[\\w\\W]+(?:\\)|\\]\\))", conf.i18n("c_count_regex"));
        assertEquals("Not supported yet.", conf.i18n("no_support"));
        assertEquals(
                "Only 1 parameter is supported at this time. e.g. count(*)|count(name) not count(x,y,.)", conf.i18n(
                        "count_single_param"));
        assertEquals("en", Configuration.DEFAULT_LANG);
        assertEquals(ResultType.XML, Configuration.DEFAULT_TYPE);
        assertEquals(false, Configuration.LOG_ENABLED);
    }
